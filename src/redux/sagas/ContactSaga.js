...
function* actGetAllContacts() {
  yield put({ type: ActionTypes.SHOW_LOADING });
  try {
    let { data, status } = yield call(() => ContactServices.getAllContacts());
    if (status === STATUS_SUCCESS) {
      yield put({
...
export function* followActGetAllContacts() {
  yield takeLatest(GET_CONTACTS_SAGA, actGetAllContacts);
}
